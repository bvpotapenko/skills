...
- `💡 A-HA #N:` — a connection point between facts. Numbered consecutively, and numbering CONTINUES across documents in the same course (Part II starts where Part I stopped). Each one earns its number: it must connect at least two previously-separate things, or reframe one thing so a later result becomes obvious. Instruct the learner to slow down and re-read these.
